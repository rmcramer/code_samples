...
class USPSBase {
    ...
    protected function getXMLString() {
        ...
        $xml = \XMLParser_RMC::createXML($this->apiCodes[$this->apiVersion], $postFields);
        return $xml->saveXML();
    }
    /**
...
/**
 */
class USPSTrack extends USPSBase {
    /**
    ...
    public function getEndpoint() {
        return self::$testMode ? parent::TEST_API_URL : parent::LIVE_API_URL;
    }
    /**
    ...
    /**
     * Determine the actual ship date from the details returned by USPS.
     *
    ...
    public function findShipDate($trackDetails)
    {
        ...
        while ($count > 0 && !$found)
        {
            ...
            if (strpos($trackDetail['Event'],'Accept') === 0 ||
                strpos($trackDetail['Event'],'Arriv') === 0 ||
                strpos($trackDetail['Event'],'Depart') === 0)
            {
                $ship_date = date("Y-m-d 08:00:00",strtotime($trackDetail['EventDate']));
                $found = true;
            }
        }
     ...
     * @return null|array
     */
    public function createPackageArray($package)
    {
        $response = [];

        if (isset($package['TrackSummary']))
        {
            $response['Service'] = 'USPS';
            $response['ID'] = $package['@attributes']['ID'];
            $response['Status'] = "Success";
            $response['Date'] = \App\Helpers::normalizeDatetimeFromUserTZtoUTC(date("Y-m-d 08:00:00",strtotime($package['TrackSummary']['EventDate'])));
            $response['Msg'] = $package['TrackSummary']['Event'];
            $response['ErrorCode'] = 0;
//                echo $package['@attributes']['ID'] . "::" . "Success::" . date("Y-m-d",strtotime($package['TrackSummary']['EventDate'])) . "::" . $package['TrackSummary']['Event'] . "\n";

            $ship_date = null;

            if (isset($package['TrackDetail']))
                $ship_date = $this->findShipDate($package['TrackDetail']);
        ...
        else
        {
            ...
            $response['Msg'] = $package['Error']['Description'];
            $response['ErrorCode'] = $package['Error']['Number'];
//                echo $package['@attributes']['ID'] . "::" . "Error::" . date("Y-m-d") . '::' . $package['Error']['Description'] . '::' . $package['Error']['Number'] . "\n";
        }
    ...
    /**
     * Find the USPS responses for an array of tracking numbers
     *    and return an array of those responses.
     *
     * @param  array $package
     *
     * @return null|array
    ...
    {
        $this->setUsername(getenv('USPS_USERNAME'));
        ...
        $response_array = $this->getArrayResponse();
        ...
        if ($response_array &&
            ...
            isset($response_array['TrackResponse']))
        {
            if (count($trackingNumbers) === 1)
            {
                $package = $response_array['TrackResponse']['TrackInfo'];
                $responses[] = self::createPackageArray($package);
            }
            else if (count($trackingNumbers) > 1)
            {
                foreach ($response_array['TrackResponse']['TrackInfo'] as $package)
                {
                    $response = self::createPackageArray($package);
                    if (is_array($response) && count($response)) $responses[] = $response;
                }
            }
        }
        else var_dump($this->getErrorMessage());
